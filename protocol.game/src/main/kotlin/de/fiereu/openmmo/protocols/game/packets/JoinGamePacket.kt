...
/**
 * Used when a player reconnects to the game server after a disconnection.
 * This data typically includes the player's unique identifier and a session key
 * that was issued by the login server to re-establish the session.
 */
data class ReconnectAuthData(
    val playerId: Long,
    val sessionKey: ByteArray
) : GameAuthenticationData()

/**
 * Used when a player is joining the game server for the first time after logging in.
 * This data typically includes the user's unique identifier and a session key
 * that was issued by the login server to authenticate the session.
 */
...
class JoinPacketSerializer : PacketSerializer<JoinGamePacket> {
    override fun serialize(packet: JoinGamePacket, buffer: ByteBuf) {
        when (packet.authData) {
            ...
            else -> throw IllegalArgumentException("Unsupported authentication data type: ${packet.authData::class.simpleName}")
        }
...
class JoinPacketDeserializer : PacketDeserializer<JoinGamePacket> {
    override fun deserialize(buffer: ByteBuf): JoinGamePacket {
        ...
        val authenticationData = when (authOption) {
            0x00.toByte() -> {
