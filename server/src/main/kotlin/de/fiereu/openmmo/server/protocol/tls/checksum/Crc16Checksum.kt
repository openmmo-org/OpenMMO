...
class Crc16Checksum : Checksum {
    ...
    override fun verify(data: ByteBuf, expectedChecksum: ByteArray): Boolean {
        if (expectedChecksum.size != 2) return false
        
        val calculatedSum = hash(data)
        val expectedSum = ((expectedChecksum[1].toInt() and 0xff) shl 8) or (expectedChecksum[0].toInt() and 0xff)
    ...
    private fun hash(data: ByteBuf): Int {
        ...
        for (i in readerIndex until data.writerIndex()) {
